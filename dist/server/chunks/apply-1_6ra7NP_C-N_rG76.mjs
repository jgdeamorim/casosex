import { c as resolveConfiguredLocale, o as getI18nConfig, r as __exportAll } from "./runner-BsI18UgP_DY3LBvkk.mjs";
...
import { t as imageSize } from "./dist_C1GPZBLn.mjs";
import { p as markContentMediaUsageCollectionStaleSafely } from "./content-refresh-DgqUBeTv_rkjORcMb.mjs";
import { i as setSiteSettings } from "./settings-DgKouY2S_DP0zmLXj.mjs";
import { a as validateExternalUrl, r as ssrfSafeFetch } from "./ssrf-CviKqWmq_Bm-KV_Vo.mjs";
...
async function applySeed(db, seed, options = {}) {
	...
	if (seed.taxonomies) {
		...
		const { invalidateTaxonomyDefsCache } = await import("./taxonomies-B61CRSha_Dfq0shOA.mjs").then((n) => n.d);
	...
	const { invalidateRedirectCache } = await import("./cache-B26cufFd_1Rjhdrw3.mjs").then((n) => n.t);
	const { invalidateUrlPatternCache } = await import("./query-DCiXI7OZ_DyF9RQWm.mjs").then((n) => n.o);
...
async function applyContentTaxonomies(db, collectionSlug, contentId, entry, isUpdate) {
	...
	if (!entry.taxonomies) {
		if (isUpdate) {
			const { invalidateTermCache } = await import("./taxonomies-B61CRSha_Dfq0shOA.mjs").then((n) => n.d);
			invalidateTermCache();
		}
		return;
	}
