...
import { l as encodeCursor, n as FOLDED_BYLINES, r as FOLDED_TERMS, t as CURSOR_RAW_VALUES } from "./loader_eyqw6jfw.mjs";
...
import { i as contentNamespaces, n as cachedQuery } from "./object-cache_BRJRfvhV.mjs";
//#region self-essentials/emdash-main/packages/core/src/visual-editing/editable.ts
/**
...
//#endregion
//#region self-essentials/emdash-main/packages/core/src/query.ts
/**
...
async function hydrateEntryBylines(type, entries) {
	...
	if (entries.every((e) => FOLDED_BYLINES in entryData(e))) {
		...
		if (!needsQueryPath) try {
			...
			const { getBylineFieldDefs } = await import("./field-defs-cache_BNy5Mqf_.mjs");
	...
	try {
		const { getBylinesForEntries } = await import("./bylines_D7EQwRuB.mjs");
...
async function hydrateEntryTerms(type, entries, locale) {
	...
	if (entries.every((e) => FOLDED_TERMS in entryData(e))) {
		...
		const { primeFoldedEntryTerms } = await import("./taxonomies_BQ4TSXEd.mjs");
		primeFoldedEntryTerms(type, perEntry, { locale });
		return;
	}
	try {
		const { getAllTermsForEntries } = await import("./taxonomies_BQ4TSXEd.mjs");
