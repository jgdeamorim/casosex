...
import { m as isValidScope } from "./passkey_eKvnpHNN.mjs";
import { n as getPublicOrigin } from "./public-url-DSGTnJFw_DEONbqkZ.mjs";
import "./api-tokens-CEsW_jCg_B5pZxyoX.mjs";
import { t as escapeHtml } from "./escape-CdW31WsG_D0HrEF5-.mjs";
import { c as validateRedirectUri, o as lookupOAuthClient, s as validateClientRedirectUri } from "./oauth-clients-BwTxE1aE_Bl65d5J5.mjs";
import "./dist_Di9U_sWQ.mjs";
import { n as handleAuthorizationApproval, t as buildDeniedRedirect } from "./oauth-authorization-C9tFinly_Cb30uuB2.mjs";
//#region self-essentials/emdash-main/packages/core/dist/astro/routes/api/oauth/authorize.mjs
...
var GET = async ({ url, request, locals }) => {
	...
	const requestedScopes = [...new Set((scope ?? "").split(" ").filter(Boolean).filter(isValidScope))];
...
var POST = async ({ request, locals }) => {
	...
	const requestedScopes = new Set((new URL(request.url).searchParams.get("scope") ?? "").split(" ").filter(Boolean).filter(isValidScope));
	const selectedScopes = [...new Set(formData.getAll("scope").filter((value) => typeof value === "string").filter(isValidScope).filter((scope) => requestedScopes.has(scope)))];
	...
	if (!result.success) {
		...
		try {
			...
			errorUrl.searchParams.set("error_description", invalidScope ? "No selected permission can be granted" : "Authorization failed");
...
function getPluginScopeLabel(scope) {
	const pluginId = scope.startsWith("mcp:tools:") ? scope.slice(10) : "";
...
//#endregion
//#region \0virtual:astro:page:self-essentials/emdash-main/packages/core/dist/astro/routes/api/oauth/authorize@_@mjs
