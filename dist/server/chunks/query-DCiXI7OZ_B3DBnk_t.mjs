import { a as getFallbackChain, o as getI18nConfig, r as __exportAll, s as isI18nEnabled } from "./runner-BsI18UgP_DY3LBvkk.mjs";
import { n as getRequestContext } from "./request-context_K9BAblf6.mjs";
import { i as contentNamespaces, n as cachedQuery, u as invalidateSchemaObjectCache } from "./object-cache-Bok5j2ae_D6Qn5mkk.mjs";
import { a as encodeCursor } from "./types-XrQQ-Aex_D1dMYdDm.mjs";
...
import { n as FOLDED_BYLINES, r as FOLDED_TERMS, t as CURSOR_RAW_VALUES } from "./loader-C1XOLV5b_C3i6HyEe.mjs";
...
async function getEmDashCollectionUncached(type, filter) {
	const { getLiveCollection } = await import("./_astro_content_CFw9CVQX.mjs");
	const ctx = getRequestContext();
	const i18nConfig = getI18nConfig();
...
async function getEmDashEntry(type, id, options) {
	const { getLiveEntry } = await import("./_astro_content_CFw9CVQX.mjs");
	const ctx = getRequestContext();
...
async function hydrateEntryBylines(type, entries) {
	...
	if (entries.every((e) => FOLDED_BYLINES in entryData(e))) {
		...
		if (!needsQueryPath) try {
			const { getDb } = await import("./loader-C1XOLV5b_BQAIjMMP.mjs").then((n) => n.o);
			const db = await getDb();
			const { getBylineFieldDefs } = await import("./field-defs-cache-QMVnzTH6_Fk1SqbQZ.mjs").then((n) => n.t);
	...
	try {
		const { getBylinesForEntries } = await import("./bylines-B8-WGdja_NYFFkMld.mjs").then((n) => n.t);
...
async function hydrateEntryTerms(type, entries, locale) {
	...
	if (entries.every((e) => FOLDED_TERMS in entryData(e))) {
		...
		const { primeFoldedEntryTerms } = await import("./taxonomies-B61CRSha_Dfq0shOA.mjs").then((n) => n.d);
		primeFoldedEntryTerms(type, perEntry, { locale });
		return;
	}
	try {
		const { getAllTermsForEntries } = await import("./taxonomies-B61CRSha_Dfq0shOA.mjs").then((n) => n.d);
...
async function getTranslations(type, id) {
	try {
		const db = (await import("./loader-C1XOLV5b_BQAIjMMP.mjs").then((n) => n.o)).getDb;
		const dbInstance = await db();
		const { ContentRepository } = await import("./content-CpfKV9QE_CtN__kj3.mjs").then((n) => n.n);
