import { a as sql, i as DialectAdapterBase, r as Migrator } from "./migrator_BiAfLowp.mjs";
import "node:module";
...
//#endregion
//#region self-essentials/emdash-main/packages/core/dist/database/pg-migration-lock.mjs
/**
...
//#endregion
//#region self-essentials/emdash-main/packages/core/dist/validate-VPnKoIzW.mjs
/**
...
//#endregion
//#region self-essentials/emdash-main/packages/core/dist/dialect-helpers-DFwyWjTM.mjs
/**
...
//#endregion
//#region self-essentials/emdash-main/packages/core/dist/config-DyF3OKg_.mjs
...
function resolveConfiguredLocale(locale) {
	return getI18nConfig()?.locales.find((configured) => configured.toLowerCase() === locale.toLowerCase()) ?? locale;
}
/**
...
//#endregion
//#region self-essentials/emdash-main/packages/core/dist/runner-BsI18UgP.mjs
...
var _054_media_upload_attempts_exports = /* @__PURE__ */ __exportAll({
	down: () => down,
	up: () => up
});
async function up(db) {
	await db.schema.createTable("_emdash_media_upload_attempts").ifNotExists().addColumn("storage_key", "text", (col) => col.primaryKey()).addColumn("media_id", "text", (col) => col.notNull()).addColumn("status", "text", (col) => col.notNull().defaultTo("active")).addColumn("created_at", "text", (col) => col.notNull().defaultTo(currentTimestamp(db))).addColumn("updated_at", "text", (col) => col.notNull().defaultTo(currentTimestamp(db))).execute();
	await db.schema.createIndex("idx_media_upload_attempts_media_id").ifNotExists().on("_emdash_media_upload_attempts").column("media_id").execute();
	await db.schema.createIndex("idx_media_upload_attempts_status_created_at").ifNotExists().on("_emdash_media_upload_attempts").columns(["status", "created_at"]).execute();
	await db.schema.createIndex("idx_media_storage_key").ifNotExists().on("media").column("storage_key").execute();
}
...
var MIGRATIONS = Object.freeze({
	...
	"054_media_upload_attempts": _054_media_upload_attempts_exports
});
...
//#endregion
export { pluginDataExtractExpr as _, getFallbackChain as a, validateIdentifier as b, resolveConfiguredLocale as c, compoundSelectLimit as d, currentTimestamp as f, listTablesLike as g, isSqlite as h, runMigrations as i, setI18nConfig as l, isPostgres as m, MIGRATION_RACE_WAIT_MS as n, getI18nConfig as o, currentTimestampValue as p, __exportAll as r, isI18nEnabled as s, ConcurrentMigrationTimeoutError as t, buildStatusCondition as u, pluginDataOrderExpr as v, validatePluginIdentifier as x, tableExists as y };
