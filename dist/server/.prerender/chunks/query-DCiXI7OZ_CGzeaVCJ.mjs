import { n as FOLDED_BYLINES, r as FOLDED_TERMS, t as CURSOR_RAW_VALUES } from "./loader-C1XOLV5b_BlhHvfIP.mjs";
import { a as getFallbackChain, o as getI18nConfig, r as __exportAll, s as isI18nEnabled } from "./runner-BsI18UgP_CTLRmh7U.mjs";
import { n as getRequestContext } from "./request-context_K9BAblf6.mjs";
import { i as contentNamespaces, l as invalidateSchemaObjectCache, n as cachedQuery } from "./object-cache-Bok5j2ae_B1rhuniT.mjs";
import { a as encodeCursor } from "./types-XrQQ-Aex_rb6o-8d1.mjs";
...
//#region self-essentials/emdash-main/packages/core/dist/query-DCiXI7OZ.mjs
/**
...
async function getEmDashCollectionUncached(type, filter) {
	const { getLiveCollection } = await import("./_astro_content_BCI_13q1.mjs");
	const ctx = getRequestContext();
	const i18nConfig = getI18nConfig();
	...
	const entriesWithEdit = pageEntries.map((entry) => {
		...
		if (!canExposeRevisionMetadata(entry, type)) stripRevisionMetadata(entry);
		return {
			...entry,
...
async function getEmDashEntry(type, id, options) {
	const { getLiveEntry } = await import("./_astro_content_BCI_13q1.mjs");
	const ctx = getRequestContext();
...
async function hydrateEntryBylines(type, entries) {
	...
	if (entries.every((e) => FOLDED_BYLINES in entryData(e))) {
		...
		if (!needsQueryPath) try {
			const { getDb } = await import("./loader-C1XOLV5b_BKH4a_n4.mjs").then((n) => n.o);
			const db = await getDb();
			const { getBylineFieldDefs } = await import("./field-defs-cache-QMVnzTH6_DSIMzO6y.mjs").then((n) => n.t);
	...
	try {
		const { getBylinesForEntries } = await import("./bylines-B8-WGdja_D8ZeFr9T.mjs").then((n) => n.t);
...
async function hydrateEntryTerms(type, entries, locale) {
	...
	if (entries.every((e) => FOLDED_TERMS in entryData(e))) {
		...
		const { primeFoldedEntryTerms } = await import("./taxonomies-B61CRSha_ovv5Cg26.mjs").then((n) => n.d);
		primeFoldedEntryTerms(type, perEntry, { locale });
		return;
	}
	try {
		const { getAllTermsForEntries } = await import("./taxonomies-B61CRSha_ovv5Cg26.mjs").then((n) => n.d);
...
async function getTranslations(type, id) {
	try {
		const db = (await import("./loader-C1XOLV5b_BKH4a_n4.mjs").then((n) => n.o)).getDb;
		const dbInstance = await db();
		const { ContentRepository } = await import("./content-CpfKV9QE_DOA8LvVw.mjs").then((n) => n.n);
...
async function resolveEmDashPath(path) {
	if (!cachedUrlPatterns) {
		const { getDb } = await import("./loader-C1XOLV5b_BKH4a_n4.mjs").then((n) => n.o);
		const { SchemaRegistry } = await import("./registry-BP1JK2xh_FblL3LUZ.mjs").then((n) => n.r);
