# CASOSEX · Constituição do Projeto & Doutrinas Duras
...
1. **DAG-First, SEMPRE:** Antes de qualquer codificação ou resposta sobre o sistema, a busca semântica cross-KG e checagem de arquivos (`/casosex-dag`) é obrigatória.
...
4. **Isolamento de Namespace:** O projeto CASOSEX opera exclusivamente com o prefixo `casosex:*` no Redis `:6396` e coleções `casosex-*` com `tag=casosex` no Qdrant `:6352`.
