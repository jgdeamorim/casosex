...
import { deleteFlowFromRedis, deleteMemoryFromRedis, deleteVariableFromRedis, getFlowFromRedis, getMemoryFromRedis, getProjectFromRedis, listFlowsFromRedis, listMemoriesFromRedis, listProjectsFromRedis, listVariablesFromRedis, saveFlowToRedis, saveMemoryToRedis, saveProjectToRedis, saveVariableToRedis, } from "../lib/redis.js";
import { computeVertexOrder, executeVertexNode, } from "../lib/execution.js";
...
const handleGetProjectById = async (c) => {
    const id = c.req.param("id") || "";
    let project = (await getProjectFromRedis(id));
...
flowsRouter.post("/build/:flow_id/vertices", async (c) => {
    ...
    if (!graph.nodes || graph.nodes.length === 0) {
        const storedFlow = (await getFlowFromRedis(flowId));
...
flowsRouter.post("/run/:flow_id", async (c) => {
    ...
    if (!graph.nodes || graph.nodes.length === 0) {
        const stored = (await getFlowFromRedis(flowId));
