...
function enrichComponentTemplates(categories) {
...
componentsRouter.get("/all", (c) => {
    return c.json(enrichComponentTemplates({
        inputs: {
            ChatInput: {
                name: "ChatInput",
        ...
        models: {
            "Gemini Scriptwriter": {
                ...
                description: "Gera roteiros otimizados (gancho + corpo + CTA) via Gemini Flash / DeepSeek V4.",
...
componentsRouter.post("/custom_component", async (c) => {
    ...
    const nodeName = body.frontend_node?.name || "CustomComponent";
    const displayName = body.frontend_node?.display_name || "Componente Customizado Volúpia";
    return c.json({
        data: {
            name: nodeName,
