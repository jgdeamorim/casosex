...
a2aRouter.get("/:flow_id/.well-known/agent-card.json", async (c) => {
    if (!isA2aEnabled()) {
        return c.json({ detail: "A2A is turned off on this server. Set LANGFLOW_A2A_ENABLED=true to publish and test agents." }, 404);
    }
    ...
    return c.json({
        name: flow.name || "Agente Volúpia A2A",
        description: flow.description || "Agente A2A Volúpia Engine",
        version: "1.0.0",
        ...
        skills: [
            {
                id: "chat",
                name: flow.name || "Volúpia Agent",
                description: flow.description || "Processa mensagens e orquestra fluxos no Volúpia Engine",
            },
        ],
    });
});
