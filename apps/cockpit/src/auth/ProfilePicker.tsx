...
export function ProfilePicker({ onSelect }: { onSelect?: () => void }): React.ReactElement {
  ...
  return (
    <div className="space-y-4" role="group" aria-label="Selecionar autenticação SSO ou perfil">
      {/* Botão Primário Google Workspace SSO */}
      <button
        type="button"
      ...
      >
        {isAuthenticating ? (
          <span className="flex items-center space-x-2 text-stone-800">
            <span className="w-4 h-4 rounded-full border-2 border-stone-800 border-t-transparent animate-spin" />
            <span>Autenticando SSO Google Workspace...</span>
          </span>
        ) : (
          <>
            ...
            <span className="tracking-tight">Entrar com Google Workspace SSO (@volupia.com.br)</span>
          </>
        )}
      </button>
      ...
      <div className="relative flex items-center justify-center my-2">
        ...
        <span className="relative px-3 bg-[#161214] text-[10px] uppercase tracking-wider text-[#a39b94] font-medium">
