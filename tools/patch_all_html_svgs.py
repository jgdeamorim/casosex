import re
import os

svg_source_path = "/home/jeffer/Downloads/ChatGPT-Image-2-de-set.-de-2026_-18_05_50.svg"
spec_files = [
    "/media/jeffer/5aab5a95-8290-d3f7-2e4f-8c27cc2d09a93/CASOSEX/docs/Brand-oficial/volupia-brand-identity-spec.html",
    "/media/jeffer/5aab5a95-8290-d3f7-2e4f-8c27cc2d09a93/CASOSEX/docs/brand-spec/volupia-brand-identity-spec.html"
]

with open(svg_source_path, "r") as f:
    svg_source = f.read()

paths = re.findall(r'd="([^"]+)"', svg_source)
inner_paths_str = "\n".join([f'    <path d="{p}"/>' for p in paths])

emblem_group_template = f'''<g transform="translate(212.250000, 1013.700000) scale(0.100000, -0.100000)" stroke="none">
{inner_paths_str}
</g>'''

for target_file in spec_files:
    if not os.path.exists(target_file):
        continue

    with open(target_file, "r") as f:
        html = f.read()

    # 1. Replace Hero Logo SVG (line 517)
    hero_pattern = r'<svg class="hero-logo-svg".*?</svg>'
    hero_replacement = f'''<svg class="hero-logo-svg" viewBox="0 0 1100 320" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="heroLipGrad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#FB7185" />
      <stop offset="40%" stop-color="#E11D48" />
      <stop offset="80%" stop-color="#9F1239" />
      <stop offset="100%" stop-color="#580A20" />
    </linearGradient>
    <linearGradient id="heroGoldGrad" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#F7EBE1" />
      <stop offset="100%" stop-color="#E5C3A6" />
    </linearGradient>
    <filter id="heroGlow" x="-10%" y="-10%" width="120%" height="120%">
      <feDropShadow dx="0" dy="4" stdDeviation="8" flood-color="#E11D48" flood-opacity="0.2" />
    </filter>
  </defs>
  <g transform="translate(30, 60) scale(0.16)" fill="url(#heroLipGrad)" filter="url(#heroGlow)">
{emblem_group_template}
  </g>
  <line x1="320" y1="65" x2="320" y2="255" stroke="#D4A373" stroke-width="1.5" stroke-linecap="round" opacity="0.45" />
  <text x="360" y="168" font-family="'Playfair Display', Georgia, serif" font-size="74" font-weight="700" letter-spacing="14" fill="url(#heroGoldGrad)">VOLÚPIA</text>
  <text x="365" y="225" font-family="'Plus Jakarta Sans', sans-serif" font-size="20" font-weight="500" letter-spacing="9" fill="#D4A373" opacity="0.9">DESPERTE SEUS SENTIDOS</text>
</svg>'''
    html = re.sub(hero_pattern, hero_replacement, html, flags=re.DOTALL)

    # 2. Replace Master Brandmark Card (Line 634)
    master_pattern = r'<svg width="120" height="120" viewBox="100 0 1336 1024" fill="none">.*?</svg>'
    master_replacement = f'''<svg width="120" height="120" viewBox="0 0 1350 1250" fill="none">
  <defs>
    <linearGradient id="masterLipGrad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#FB7185" />
      <stop offset="35%" stop-color="#E11D48" />
      <stop offset="75%" stop-color="#9F1239" />
      <stop offset="100%" stop-color="#4C0519" />
    </linearGradient>
  </defs>
  <g fill="url(#masterLipGrad)">
{emblem_group_template}
  </g>
</svg>'''
    html = re.sub(master_pattern, master_replacement, html, count=1, flags=re.DOTALL)

    # 3. Replace Monochrome Brandmark Card (Line 653)
    mono_pattern = r'<svg width="120" height="120" viewBox="100 0 1336 1024" fill="none">.*?</svg>'
    mono_replacement = f'''<svg width="120" height="120" viewBox="0 0 1350 1250" fill="none">
  <g fill="#D4A373">
{emblem_group_template}
  </g>
</svg>'''
    html = re.sub(mono_pattern, mono_replacement, html, count=1, flags=re.DOTALL)

    # 4. Replace Primary Stacked Lockup Card (Line 728)
    stacked_pattern = r'<svg width="280" height="180" viewBox="0 0 1000 800" fill="none">.*?</svg>'
    stacked_replacement = f'''<svg width="280" height="180" viewBox="0 0 800 720" fill="none">
  <defs>
    <linearGradient id="stackedLipGrad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#FB7185" />
      <stop offset="40%" stop-color="#E11D48" />
      <stop offset="80%" stop-color="#9F1239" />
      <stop offset="100%" stop-color="#580A20" />
    </linearGradient>
    <linearGradient id="goldTextGradStack" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#F7EBE1" />
      <stop offset="100%" stop-color="#E5C3A6" />
    </linearGradient>
  </defs>
  <g transform="translate(220, 40) scale(0.2667)" fill="url(#stackedLipGrad)">
{emblem_group_template}
  </g>
  <line x1="320" y1="410" x2="480" y2="410" stroke="#D4A373" stroke-width="1.5" stroke-linecap="round" opacity="0.4" />
  <text x="400" y="505" font-family="'Playfair Display', Georgia, serif" font-size="72" font-weight="700" letter-spacing="16" fill="url(#goldTextGradStack)" text-anchor="middle">VOLÚPIA</text>
  <text x="400" y="565" font-family="'Plus Jakarta Sans', sans-serif" font-size="20" font-weight="500" letter-spacing="10" fill="#D4A373" text-anchor="middle" opacity="0.9">DESPERTE SEUS SENTIDOS</text>
</svg>'''
    html = re.sub(stacked_pattern, stacked_replacement, html, flags=re.DOTALL)

    # 5. Replace Horizontal Lockup Card (Line 758)
    hz_pattern = r'<svg width="340" height="100" viewBox="0 0 1200 360" fill="none">.*?</svg>'
    hz_replacement = f'''<svg width="340" height="100" viewBox="0 0 1100 320" fill="none">
  <defs>
    <linearGradient id="hzCardGrad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#FB7185" />
      <stop offset="40%" stop-color="#E11D48" />
      <stop offset="80%" stop-color="#9F1239" />
      <stop offset="100%" stop-color="#580A20" />
    </linearGradient>
    <linearGradient id="goldTextGradHz" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#F7EBE1" />
      <stop offset="100%" stop-color="#E5C3A6" />
    </linearGradient>
  </defs>
  <g transform="translate(30, 60) scale(0.16)" fill="url(#hzCardGrad)">
{emblem_group_template}
  </g>
  <line x1="320" y1="65" x2="320" y2="255" stroke="#D4A373" stroke-width="1.5" stroke-linecap="round" opacity="0.45" />
  <text x="360" y="168" font-family="'Playfair Display', Georgia, serif" font-size="74" font-weight="700" letter-spacing="14" fill="url(#goldTextGradHz)">VOLÚPIA</text>
  <text x="365" y="225" font-family="'Plus Jakarta Sans', sans-serif" font-size="20" font-weight="500" letter-spacing="9" fill="#D4A373" opacity="0.9">DESPERTE SEUS SENTIDOS</text>
</svg>'''
    html = re.sub(hz_pattern, hz_replacement, html, flags=re.DOTALL)

    # 6. Replace Favicons (Line 852, 868, 884)
    fav_pattern36 = r'<svg width="36" height="36" viewBox="100 0 1336 1024" fill="none">.*?</svg>'
    fav_replacement36 = f'''<svg width="36" height="36" viewBox="0 0 1350 1250" fill="none">
  <g fill="#D4A373">
{emblem_group_template}
  </g>
</svg>'''
    html = re.sub(fav_pattern36, fav_replacement36, html, flags=re.DOTALL)

    fav_pattern20 = r'<svg width="20" height="20" viewBox="100 0 1336 1024" fill="none">.*?</svg>'
    fav_replacement20 = f'''<svg width="20" height="20" viewBox="0 0 1350 1250" fill="none">
  <g fill="#D4A373">
{emblem_group_template}
  </g>
</svg>'''
    html = re.sub(fav_pattern20, fav_replacement20, html, flags=re.DOTALL)

    fav_pattern10 = r'<svg width="10" height="10" viewBox="100 0 1336 1024" fill="none">.*?</svg>'
    fav_replacement10 = f'''<svg width="10" height="10" viewBox="0 0 1350 1250" fill="none">
  <g fill="#FAF7F5">
{emblem_group_template}
  </g>
</svg>'''
    html = re.sub(fav_pattern10, fav_replacement10, html, flags=re.DOTALL)

    # 7. Replace E-commerce Header Simulator (Line 910)
    sim_pattern = r'<svg width="320" height="90" viewBox="0 0 1200 360" fill="none">.*?</svg>'
    sim_replacement = f'''<svg width="320" height="90" viewBox="0 0 1100 320" fill="none">
  <defs>
    <linearGradient id="simLipGrad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#FB7185" />
      <stop offset="40%" stop-color="#E11D48" />
      <stop offset="80%" stop-color="#9F1239" />
      <stop offset="100%" stop-color="#580A20" />
    </linearGradient>
    <linearGradient id="goldTextGradSim" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#F7EBE1" />
      <stop offset="100%" stop-color="#E5C3A6" />
    </linearGradient>
  </defs>
  <g transform="translate(30, 60) scale(0.16)" fill="url(#simLipGrad)">
{emblem_group_template}
  </g>
  <line x1="320" y1="65" x2="320" y2="255" stroke="#D4A373" stroke-width="1.5" stroke-linecap="round" opacity="0.45" />
  <text x="360" y="168" font-family="'Playfair Display', Georgia, serif" font-size="74" font-weight="700" letter-spacing="14" fill="url(#goldTextGradSim)">VOLÚPIA</text>
  <text x="365" y="225" font-family="'Plus Jakarta Sans', sans-serif" font-size="20" font-weight="500" letter-spacing="9" fill="#D4A373" opacity="0.9">DESPERTE SEUS SENTIDOS</text>
</svg>'''
    html = re.sub(sim_pattern, sim_replacement, html, flags=re.DOTALL)

    with open(target_file, "w") as f:
        f.write(html)

print("HTML com alinhamento óptico e harmonia de cores aplicados em todas as instâncias!")
