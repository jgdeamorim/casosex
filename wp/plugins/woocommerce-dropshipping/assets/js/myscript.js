jQuery(function ($) {
	if ($.fn.datepicker) {
		$("#input-dialog-date").datepicker({ dateFormat: 'yy-mm-dd' });
	}
});

function open_dropshipper_dialog(my_id) {

	var container = jQuery("#dropshipper_shipping_info_" + my_id);

	jQuery("#input-dialog-date").val(container.find(".dropshipper_date").html());
	jQuery("#input-dialog-trackingnumber").val(container.find(".dropshipper_tracking_number").html());
	jQuery("#input-dialog-shippingcompany").val(container.find(".dropshipper_shipping_company").html());
	jQuery("#input-dialog-notes").val(container.find(".dropshipper_notes").html());
...
jQuery(document).ready(function () {
	...
	jQuery("#generate_ali_key").click(function () {
		var data = {
			...
			security: wc_dropshipping.security // send nonce from PHP
		};

		jQuery.post(wc_dropshipping.ajax_url, data, function (response) {
...
jQuery(document).ready(function ($) {
	...
	jQuery(document).on('change', '.bar_cal', function () {		
		...
		var dynamic_value = jQuery('#profit_margin_hidden').val();
		var allelements = dynamic_value.split('~');
		if (!$('#profit_percent_value').prop('disabled') || !$('#profit_doller_value').prop('disabled')) {
			if (prft_prcnt_val !== '') {
		...
		} else if (dynamic_value !== '' && $('#profit_percent_value').prop('disabled') && $('#profit_doller_value').prop('disabled')) {
			var isInRange = false;
			allelements.forEach(function (row) {
				var alltds = row.split('_');
				if (100 >= alltds[0] && 100 <= alltds[1]) {
					isInRange = true;
