...
if ( ! class_exists( 'WC_Dropshipping_Dashboard', false ) ) :
	...
	class WC_Dropshipping_Dashboard {
		...
		public function __construct() {
			...
			// Get today's date in WP timezone at 00:00:00
		...
		public function custom_dashboard_style() {
			...
			wp_register_script( 'add_custom_dashboard_script', plugins_url() . '/' . $base_name[0] . '/assets/js/dashboard.js', array( 'add_dropshipping_chart_lib', 'jquery' ), '1.0.0' );
		}
		...
		public function get_one_week_daily_total_orders() {
			...
			// Generate the dates
			$date_7 = clone $today;
			$date_7->modify('-6 days');
			$date_6 = clone $today;
			$date_6->modify('-5 days');
			$date_5 = clone $today;
			$date_5->modify('-4 days');
			$date_4 = clone $today;
			$date_4->modify('-3 days');
			$date_3 = clone $today;
			$date_3->modify('-2 days');
			$date_2 = clone $today;
			$date_2->modify('-1 day');
			$date_1 = $today;

			$date_7 = $date_7->format('Y-m-d 00:00:00');
			$date_6 = $date_6->format('Y-m-d 00:00:00');
			$date_5 = $date_5->format('Y-m-d 00:00:00');
			$date_4 = $date_4->format('Y-m-d 00:00:00');
			$date_3 = $date_3->format('Y-m-d 00:00:00');
			$date_2 = $date_2->format('Y-m-d 00:00:00');
			$date_1 = $date_1->format('Y-m-d 00:00:00');

			$this->logger->info( print_r( $date_7, true ), array( 'source' => 'woocommerce-dropshipping' ) );
			$this->logger->info( print_r( $date_6, true ), array( 'source' => 'woocommerce-dropshipping' ) );
			$this->logger->info( print_r( $date_5, true ), array( 'source' => 'woocommerce-dropshipping' ) );
			$this->logger->info( print_r( $date_4, true ), array( 'source' => 'woocommerce-dropshipping' ) );
			$this->logger->info( print_r( $date_3, true ), array( 'source' => 'woocommerce-dropshipping' ) );
			$this->logger->info( print_r( $date_2, true ), array( 'source' => 'woocommerce-dropshipping' ) );
			$this->logger->info( print_r( $date_1, true ), array( 'source' => 'woocommerce-dropshipping' ) );

			/*
			error_log(print_r($date_7, true));
			error_log(print_r($date_6, true));
			error_log(print_r($date_5, true));
			error_log(print_r($date_4, true));
			error_log(print_r($date_3, true));
			error_log(print_r($date_2, true));
			error_log(print_r($date_1, true));
			*/
		...
		public function get_per_week_total_orders() {
			...
			$wp_timezone = new DateTimeZone($timezone_string ?: 'UTC');

			$date_5 = (new DateTime('5 weeks ago monday', $wp_timezone))->format('Y-m-d 00:00:00');
			$date_4 = (new DateTime('4 weeks ago monday', $wp_timezone))->format('Y-m-d 00:00:00');
			$date_3 = (new DateTime('3 weeks ago monday', $wp_timezone))->format('Y-m-d 00:00:00');
			$date_2 = (new DateTime('last week', $wp_timezone))->format('Y-m-d 00:00:00');
			$date_1 = (new DateTime('this week', $wp_timezone))->format('Y-m-d 00:00:00');
		...
		public function get_one_month_total_orders()
		{
			...
			$date_5 = $now->modify('-4 months')->format('Y-m-d 00:00:00');
			$date_4 = $now->modify('-3 months')->format('Y-m-d 00:00:00');
			$date_3 = $now->modify('-2 months')->format('Y-m-d 00:00:00');
			$date_2 = $now->modify('-1 months')->format('Y-m-d 00:00:00');
			$date_1 = $now->format('Y-m-d 00:00:00');
		...
		public function get_per_year_total_orders() {
			...
			$date_5 = (new DateTime('-4 year', $timezone))->format('Y-m-d 00:00:00');
			$date_4 = (new DateTime('-3 year', $timezone))->format('Y-m-d 00:00:00');
			$date_3 = (new DateTime('-2 year', $timezone))->format('Y-m-d 00:00:00');
			$date_2 = (new DateTime('-1 year', $timezone))->format('Y-m-d 00:00:00');
			$date_1 = (new DateTime('now', $timezone))->format('Y-m-d 00:00:00');
		...
		public function get_days_in_month( $month, $year ) {
			...
			return $days_in_month[ $month ] ?? 0; // Return 0 for invalid months
		}
		...
		public function get_week_total_sales() {
			...
			$days_last_month = $this->get_days_in_month( $last_month, $current_year );
			...
			foreach ( $order_ids as $order_id ) {
				...
				$date_paid_temp = opmc_hpos_get_post_meta( $order_id, '_date_paid' );
		...
		public function get_daily_total_sales() {
			...
			$datetime = new DateTime('yesterday', wp_timezone());
			$last_date = $datetime->format('d');
			...
			foreach ( $order_ids as $key => $value ) {
				...
				$total_shipping_price = $order_total->get_shipping_total();
				$cost_of_goods_total = $cost_of_goods_total + $cost_of_goods;
				$total_tax = (float) $total_tax + (float) $total_tax_price;
				$total_shipping = (float) $total_shipping + (float) $total_shipping_price;
				$last_day_orders_data[ $current_date ] = $total_sales_per_day;
