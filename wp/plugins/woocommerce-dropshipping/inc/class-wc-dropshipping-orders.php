...
class WC_Dropshipping_Orders {
	...
	public function init() {
		...
		add_action( 'woocommerce_order_action_resend_dropship_supplier_notifications', array( $this, 'manual_resend_supplier_notifications' ) );
		add_action( 'wc_dropship_manager_send_order', array( $this, 'send_order' ), 10, 3 );
		add_filter( 'wp_mail_content_type', array( $this, 'wpse27856_set_content_type' ) );
		add_action( 'woocommerce_email_before_order_table', array( $this, 'add_tracking_info_customer_email' ), 10, 3 );
		add_filter( 'woocommerce_order_item_get_formatted_meta_data', array( $this, 'order_item_get_formatted_meta_data' ), 10, 1 );		
		add_action( 'woocommerce_checkout_create_order_line_item', array( $this, 'store_product_meta' ), 10, 4 );
		/* Filter by Supplier */
	...
	function round_price_product( $price ) {
		...
		return wc_format_decimal( $rounded_price );
	}
	...
	public function order_complete( $order_id ) {
		...
		$complete_email = isset( $dropship_data['complete_email'] ) ? $dropship_data['complete_email'] : '';
		$fullinfo       = isset( $dropship_data['full_information'] ) ? $dropship_data['full_information'] : '';
	...
	public function manual_resend_supplier_notifications( $order_id ) {
		$order = new WC_Order( $order_id ); // load the order from woocommerce
		$this->notify_warehouse( $order, true ); // notify the warehouse to ship the order
	}
	...
	public function notify_warehouse( $order, $manual = false ) {
		...
		foreach ( $supplier_codes as $code => $supplier_info ) {
			...
			do_action( 'wc_dropship_manager_send_order', $order_info, $supplier_info, $manual );
		}
	...
	public function get_order_product_info( $item, $product ) {
		...
		// Product Variations
		$info['variation_data'] = array();
		$info['variation_labels'] = array();
		$info['variation_name'] = array();
	...
	public function get_order_info( $order ) {
		$options = get_option( 'wc_dropship_manager' );
		$hideorderdetail_suppliername = isset( $options['hideorderdetail_suppliername'] ) ? $options['hideorderdetail_suppliername'] : '';
		$woocommerce_price_decimal_sep = get_option( 'woocommerce_price_decimal_sep' );
		$woocommerce_price_thousand_sep = get_option( 'woocommerce_price_thousand_sep' );
		...
		if ( count( $items ) > 0 ) {
			foreach ( $items as $item_id => $item ) {
				...
				if ( $supid != '' || ! is_null( $supid ) ) {
					// Store on the order item (HPOS-safe). Keep legacy post meta for older readers.
					wc_update_order_item_meta( $item_id, 'supplierid', $supid );
	...
	public function make_csv( $order_info, $supplier_info, $html, $file_name ) {
		...
		if ( $options['csv_inmail'] == '1' ) {
			...
			if ( ! empty( $order_shipping_address_2 ) ) {
				$full_ship_address = $order_shipping_address_1 . ', ' . $order_shipping_address_2 . ', ' . $order_shipping_city . ', ' . $order_shipping_state . ', ' . $order_shipping_postcode . ', ' . $order_shipping_country;
			} else {
				$full_ship_address = $order_shipping_address_1 . ', ' . $order_shipping_city . ', ' . $order_shipping_state . ', ' . $order_shipping_postcode . ', ' . $order_shipping_country;
			}
			...
			foreach ( $order_info[ $supplier_info['slug'] ] as $prod_info ) {
				...
				fputcsv(
					$file,
					array(
						htmlspecialchars( $prod_info['name'], ENT_COMPAT ),
						htmlspecialchars( $prod_info['sku'], ENT_COMPAT ),
						htmlspecialchars( $prod_info['qty'], ENT_COMPAT ),
						html_entity_decode( strip_tags( $prod_info['price'] ), ENT_COMPAT, 'UTF-8' ),
						$product_description,
	...
	public function get_template_html( $templatepath, $order_info, $supplier_info ) {
		...
		if ( file_exists( $templatepath ) ) {
			include $templatepath; // nosemgrep: audit.php.lang.security.file.inclusion-arg
		} else {
			echo '<b>Template ' . esc_html( $templatepath ) . ' not found!</b>';
		}
		$html = ob_get_clean();
		return $html;
	}
	...
	public function send_order( $order_info, $supplier_info, $manual = false ) {
		$options = get_option( 'wc_dropship_manager' );
		$order = wc_get_order( $order_info['id'] );
		...
		if ( function_exists( 'wcs_get_subscriptions_for_renewal_order' ) ) {
			...
			if ( empty( $renewal_order ) ) {
				$this->_send_order( $order_info, $supplier_info, $manual );
	...
	public function _send_order( $order_info, $supplier_info, $manual = false ) {
		...
		$order_email = $options['view_order'];
		$cnf_mail = $options['cnf_mail'];
		$encoded_attachment = null;
		$encoded_attachment_csv = null;

		$auto_disabled  = isset( $options['supp_notification'] ) && $options['supp_notification'] == 1;
		$allow_override = ! isset( $options['allow_manual_supplier_notification'] ) || ! empty( $options['allow_manual_supplier_notification'] );
		...
		if ( $smtp_check == '1' || $std_mail == '1' || $std_mail == '' ) {
			...
			if ( $order->get_status() == 'processing' ) {
				if ( $complete_url == '1' ) {
			    	...
			    	<tr><td style="text-align: center;">' . sprintf( __( 'To mark this order as shipped please click the following link:', 'woocommerce-dropshipping' ), '' ) . '<br/><a href="' . get_admin_url() . 'admin-ajax.php?action=woocommerce_dropshippers_mark_as_shipped&orderid=' . $order_info['custom_order_number'] . '&supplierid=' . $supplier_info['id'] . '">' . sprintf( __( 'Mark as shipped', 'woocommerce-dropshipping' ), '' ) . '</a></td></tr></table>';
				}
				if ( $order_email == '1' ) {
					$message .= '<table cellpadding="8" cellspacing="0" style="width:100%;" >
			    	<tr><td style="text-align: center;"><a href="' . get_admin_url() . 'admin.php?page=dropshipper-order-list">' . sprintf( __( 'View order in dashboard', 'woocommerce-dropshipping' ), '' ) . '</a></td></tr></table>';
				}
			}
			if ( wc_dropshipping_is_open_track_enabled() ) {
				$message .= wc_dropshipping_get_mail_track_pixel_html( $order_info['id'], $supplier_info['id'] );
			}


			$headers = $this->get_supplier_html_mail_headers( $from_name, $from_email, $cc_email );
			if ( isset( $options['supp_notification'] ) ) {
				if ( $auto_disabled && ( ! $manual || ! $allow_override ) ) {
					...
					$this->logger->info( print_r( $order->get_order_number(), true ), array( 'source' => 'woocommerce-dropshipping' ) );
					$this->logger->info( 'Order notification disabled - will not send notification to supplier', array( 'source' => 'woocommerce-dropshipping' ) );
					return;
				}
			}
			$mail_sent = false;
			if ( $order->get_status() == 'completed' || $order->get_status() == 'cancelled' ) {
				...
				$this->logger->info( 'Order Status ' . $order->get_status(), array( 'source' => 'woocommerce-dropshipping' ) );
				$mail_sent = wp_mail( $hdrs['To'], $hdrs['Subject'], $message, $headers );
			} else {

				//error_log( 'Order Status ' . $order->get_status() );
				$this->logger->info( 'Order Status ' . $order->get_status(), array( 'source' => 'woocommerce-dropshipping' ) );
				$mail_sent = wp_mail( $hdrs['To'], $hdrs['Subject'], $message, $headers, $attachments );
			}

			if ( $manual && $mail_sent ) {
				$this->add_manual_supplier_notification_note( $order, $hdrs['To'] );
			}
		} else {
			...
			if ( isset( $attachments[ $file_name ] ) ) {
				$encoded_attachment = chunk_split( base64_encode( file_get_contents( $attachments[ $file_name ] ) ) );
			}
			if ( isset( $attachments[ $csv_name ] ) ) {
				$encoded_attachment_csv = chunk_split( base64_encode( file_get_contents( $attachments[ $csv_name ] ) ) );
			}
			$hdrs = array();
			$hdrs['From'] = $from_email;
			$hdrs['To'] = $supplier_info['order_email_addresses'];
			...
			if ( $options['view_order'] == '1' ) {
				$order_link = '<div style="text-align: center; margin-top: -15px;padding-bottom: 10px;"><a href="' . get_admin_url() . 'admin.php?page=dropshipper-order-list" style="display: inline-block; font-size:16px; font-weight: bold; color: green; text-decoration: none;">' . sprintf( __( 'View order in dashboard', 'woocommerce-dropshipping' ), '' ) . '</a></div>';
			} else {
				$order_link = '';
			}
			$email_message_html = '<div style="background-color: ' . ( ! empty( trim( get_option( 'woocommerce_email_background_color' ) ) ) ? get_option( 'woocommerce_email_background_color' ) : '#ccc' ) . ';">';
			$email_message_html .= $html . $order_link;
			if ( wc_dropshipping_is_open_track_enabled() ) {
				$email_message_html .= wc_dropshipping_get_mail_track_pixel_html( $order_info['id'], $supplier_info['id'] );
			}
			$email_message_html .= '</div>';
			...
			if ( isset( $options['supp_notification'] ) ) {
				if ( $auto_disabled && ( ! $manual || ! $allow_override ) ) {
					...
					$this->logger->info( print_r( $order->get_order_number(), true ), array( 'source' => 'woocommerce-dropshipping' ) );
					$this->logger->info( 'Order notification disabled â€” will not send notification to supplier', array( 'source' => 'woocommerce-dropshipping' ) );

					return;
				}
			}
			$mail_sent = wp_mail( $hdrs['To'], $hdrs['Subject'], $email_message_html, $this->get_supplier_html_mail_headers( $from_name, $from_email, $cc_email ), $attachments );
			if ( $manual && $mail_sent ) {
				$this->add_manual_supplier_notification_note( $order, $hdrs['To'] );
			}
	...
	private function get_supplier_html_mail_headers( $from_name, $from_email, $cc_email ) {
		$headers  = 'From: ' . wp_specialchars_decode( $from_name ) . ' <' . $from_email . ">\r\n";
	...
	private function add_manual_supplier_notification_note( $order, $supplier_email ) {
		$timestamp = current_time( 'mysql' );
		$order->add_order_note(
			sprintf(
				/* translators: 1: supplier email addresses, 2: timestamp. */
				__( 'Supplier notification manually sent by admin. Supplier email: %1$s. Timestamp: %2$s.', 'woocommerce-dropshipping' ),
				$supplier_email,
				$timestamp
			)
		);
	...
	public function send_order_attach_packingslip( $attachments, $order_info, $supplier_info ) {
		...
		if ( $fullinfo == '1' ) {
			$file_name = $order_info['number'] . '_' . $supplier_info['slug'] . '.pdf';
			$attachments[ $file_name ] = $this->make_pdf( $order_info, $supplier_info, $html, $file_name );  // create a pdf packing slip file
		}
		if ( $options['csv_inmail'] == '1' ) {
			$attachments[ $csv_name ] = $this->make_csv( $order_info, $supplier_info, $html, $csv_name );
		}
		return $attachments;
	}
				...
				if ( $status === 'completed' ) {
					...
					foreach ( $uniqe_userid as $key => $value ) {
						...
						if ( $dropshipper_shipping_info ) {
							echo '<p style="color: #96588a; display: block; font-size: 16px !important; font-weight: bold !important; line-height: 130%; text-align: left;">Tracking Information</p>';
							echo '<div style="margin-bottom: 20px; display: flex; flex-wrap: wrap; gap: 10px; border: 1px solid #ddd; padding: 10px; font-family: Arial, sans-serif;">';

							// Date
							$calendar_icon = esc_url( plugin_dir_url( __FILE__ ) . '../assets/icons/calendar.png' );

							echo '<div class="calendar_class" style="margin-right: 25px; padding: 8px; background-color: #fff; display: flex; align-items: center;">';
							echo '<img src="' . esc_url( $calendar_icon ) . '" alt="Calendar Icon" style="width: 18px; height: 18px; margin-right: 8px;">';
							echo '<div>';
							echo '<strong>Date</strong>';
							echo '<div>' . ( ! empty( $dropshipper_shipping_info['date'] ) ? esc_html( $dropshipper_shipping_info['date'] ) : 'N/A' ) . '</div>';
							echo '</div>';
							echo '</div>';

							// Tracking Number
							$order_tracking_icon = esc_url( plugin_dir_url( __FILE__ ) . '../assets/icons/number.png' );

							echo '<div style="margin-right: 50px; padding: 8px; background-color: #f9f9f9; display: flex; align-items: center;">';
							echo '<img src="' . esc_url( $order_tracking_icon ) . '" alt="Order Tracking Icon" style="width: 18px; height: 18px; margin-right: 8px;">';
							echo '<div>';
							echo '<strong>Tracking Number</strong>';
							echo '<div>' . ( ! empty( $dropshipper_shipping_info['tracking_number'] ) ? esc_html( $dropshipper_shipping_info['tracking_number'] ) : 'N/A' ) . '</div>';
							echo '</div>';
							echo '</div>';

							$delivery_icon = esc_url( plugin_dir_url( __FILE__ ) . '../assets/icons/delivery.png' );
							// Shipping Company
							echo '<div style="padding: 8px; background-color: #fff; display: flex; align-items: center;">';
							echo '<img src="' . esc_url( $delivery_icon ) . '" alt="Order Tracking Icon" style="width: 18px; height: 18px; margin-right: 8px;">';
							echo '<div>';
							echo '<strong>Shipping Company</strong>';
							echo '<div style="color: #96588a;">' . ( ! empty( $dropshipper_shipping_info['shipping_company'] ) ? esc_html( $dropshipper_shipping_info['shipping_company'] ) : 'N/A' ) . '</div>';
							echo '</div>';
							echo '</div>';
	...
	/**
	 * Store product meta information for dropship suppliers in the order meta.
	 *
	 * This function is used to add the dropship supplier information associated with
	 * a product to the order meta data. It iterates through the dropship suppliers
	 * assigned to the product and stores each supplier's term ID in the order meta
	 * with a unique key.
	 *
	 * @param WC_Order_Item_Product $item The order item object containing product data.
	 * @param string                $cart_item_key The unique key for the cart item.
	 * @param array                 $values The cart item data.
	 * @param WC_Order              $order The order object.
	 */
	public function store_product_meta( $item, $cart_item_key, $values, $order ) {
		$product_id = $item->get_product_id();
		$terms = get_the_terms( $product_id, 'dropship_supplier' );
		if ( ! empty( $terms ) ) {
			foreach ( $terms as $term ) {
				$term_id = $term->term_id;
				$order->update_meta_data( 'dropship_supplier_' . $term_id, $term_id );
			}
		}
	}
	...
	public function admin_shop_order_by_supplier_meta_filter() {
		...
		if ( 'shop_order' === $typenow && 'edit.php' === $pagenow ) {
			...
			$current   = isset( $_GET[ $filter_id ] ) ? $_GET[ $filter_id ] : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

			echo '<select name="' . esc_attr( $filter_id ) . '">
	        <option value="">' . esc_html__( 'Filter by Supplier', $domain ) . '</option>';
	...
	public function process_admin_shop_order_supplier_by_filter( $query ) {
		...
		if ( $query->is_admin && $pagenow === 'edit.php' && $post_type === 'shop_order' && isset( $_GET[ $filter_id ] ) && $_GET[ $filter_id ] != '' ) {
			...
			if ( isset( $_GET[ $filter_id ] ) && '' !== $_GET[ $filter_id ] ) {
				// wp_unslash() because WP may have added slashes, then sanitize_text_field()
				$supplier_val = sanitize_text_field( wp_unslash( $_GET[ $filter_id ] ) );
			}
			...
			$order_ids = $wpdb->get_col( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
