...
class WC_Dropshipping_CSV_Import {
	...
	public function CSV_upload_form() {
		$term_id = $_GET['term_id']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$ds = wc_dropshipping_get_dropship_supplier(intval($term_id));
		// Inventory Upload window		
		echo '<div id="CSVwindow">
			<form class="csvupload_form" action="' . esc_url(admin_url('admin-ajax.php')) . '" method="post" enctype="multipart/form-data" target="csvupload_iframe-' . esc_attr($ds['slug']) . '" >';
				...
				echo '<table>
						...
						<tr>
							<td>';
								echo '<input type="hidden" id="csv_upload_term_id" name="term_id" value="' . htmlspecialchars($term_id, ENT_QUOTES, 'UTF-8') . '" />'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								echo '<input type="file" name="csv_file" value="" />';
								echo '<input type="hidden" value="' . esc_attr($ds['slug']) . '" name="slug">';
			...
			echo '<iframe style="display:none;width: 100%;height: 340px" id="csvupload_iframe-' . esc_attr($ds['slug']) . '" class="csv_upload_iframe" name="csvupload_iframe-' . esc_attr($ds['slug']) . '" src="#" ></iframe>
		</div>';
		wp_die();
	}
	...
	public function admin_save_inventory_status() {
		...
		$q_select_skus = 'CREATE TEMPORARY TABLE %name AS ( SELECT m.post_id FROM ' . $wpdb->postmeta . ' m
		...
		$q_update_stockstatus = ' UPDATE ' . $wpdb->postmeta . "
			...
			AND post_id IN ( SELECT post_id AS id FROM %name );";
		...
		if ($ds['csv_column_sku'] == '' || $ds['csv_column_qty'] == '') {
			echo '<p>The settings of the supplier are incorrect. Please check and correct the settings.</p>';
		}
		// process uploaded CSV
		if (($_FILES['csv_file']['error'] == 0) && (strlen($ds['csv_delimiter']) > 0)) {

			$name = $_FILES['csv_file']['name']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			$ext_file = explode('.', $name);
			$ext = strtolower(end($ext_file));
			$type = $_FILES['csv_file']['type']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			$tmpName = $_FILES['csv_file']['tmp_name']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			if ($ext === 'csv') {
				...
				if (($handle = fopen($tmpName, 'r')) !== false) {
					...
					while (($data = fgetcsv($handle, 1000, $ds['csv_delimiter'], '"', '\\')) !== false) {
						$temp = array();
						$sku_index = (int) $ds['csv_column_sku'] - 1;

						if (isset($data[$sku_index])) {
							$temp['sku'] = $data[$sku_index];
						}

						$csv_column_qty = isset($ds['csv_column_qty']) ? (int) $ds['csv_column_qty'] : 0;
						$csv_column_indicator = isset($ds['csv_column_indicator']) ? (int) $ds['csv_column_indicator'] : 0;

						if ($ds['csv_type'] === 'quantity') {
							$temp['qty_remaining'] = preg_replace('/[^0-9]/', '', $data[$csv_column_qty - 1] ?? '');							
							...
							if ((int) trim($temp['qty_remaining']) < $options['inventory_pad']) {
							...
							if (strcasecmp(trim($data[$csv_column_indicator - 1] ?? ''), $ds['csv_indicator_instock']) != 0) {
								// if the field does not equal the "in-stock" indicator then its out of stock
								echo '<p>csv_indicator_instock</p>';
								$outofstock .= "'$temp[sku]',";
							} else {
								echo '<p>csv_indicator_instock roduct is active</p>';
								// product is active
								$instock .= "'$temp[sku]',";
							}
						}
					...
					if (strlen($outofstock) > 0) {
						$sql = str_replace('%name', 'outofstock_skus', $q_select_skus); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$sql = str_replace('%s', $outofstock, $sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$wpdb->query($sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared

						// use temp table to update stock status on all skus that are oos
						$sql = str_replace('%name', 'outofstock_skus', $q_update_stockstatus); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$sql = $wpdb->prepare($sql, array('outofstock')); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$wpdb->query($sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared

						// use temp table to update visibility on all skus that are oos
						$sql = str_replace('%name', 'outofstock_skus', $q_update_visibilitystatus); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$sql = $wpdb->prepare($sql, array('hidden')); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$wpdb->query($sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					}

					if (strlen($instock) > 0) {
						// update all now instock skus
						$sql = str_replace('%name', 'instock_skus', $q_select_skus); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$sql = str_replace('%s', $instock, $sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$wpdb->query($sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared

						$sql = str_replace('%name', 'instock_skus', $q_update_stockstatus); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$sql = $wpdb->prepare($sql, array('instock')); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$wpdb->query($sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared

						// use temp table to update visibility on all skus that are in stock
						$sql = str_replace('%name', 'instock_skus', $q_update_visibilitystatus); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$sql = $wpdb->prepare($sql, array('visible')); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$wpdb->query($sql); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					}
				...
				echo '<p>The uploaded file is not in CSV format. Please upload a CSV file.</p>';
			}
		} else {
	...
	public function woo_dropship_add_columns($columns) {
		$columns['dropship_supplier_id'] = esc_html__('Supplier Id', 'woocommerce-dropshipping');
		$columns['dropship_supplier_name'] = esc_html__('Supplier Name', 'woocommerce-dropshipping');
		$columns['dropship_supplier_slug'] = esc_html__('Supplier Slug', 'woocommerce-dropshipping');
		$columns['dropship_supplier_description'] = esc_html__('Supplier Description', 'woocommerce-dropshipping');
		$columns['dropship_supplier_email'] = esc_html__('Supplier Email', 'woocommerce-dropshipping');
		$columns['dropship_supplier_account_number'] = esc_html__('Supplier Account Number', 'woocommerce-dropshipping');
		return $columns;
	}
	...
	function woo_dropship_add_columns_to_mapping_screen($columns)
	{
		...
		$columns[esc_html__('Supplier Id', 'woocommerce-dropshipping')] = 'dropship_supplier_id';
					...
					foreach ($bigArr as $arr) {
						...
						if ($term !== 0 && $term !== null) {
							...
							if (false == email_exists($arr['email'])) {
								...
								$random_password = esc_html__('User already exists.  Password inherited.', 'woocommerce-dropshipping');
							}
						}
					}
				}
	...
	function woo_dropship_set_taxonomy_update($row, $mapped_keys) {
		...
		if (! empty($productSku)) {
			...
			$this->logger->info( 'productId ' . $pId, array( 'source' => 'woocommerce-dropshipping' ) );

			// If product found
			if ($pId > 0) {
				if (!empty($row[$dropship_supplier_slug_key])) {
					// Process and assign supplier
					$dropship_supplier_slug = sanitize_title(strip_tags($row[$dropship_supplier_slug_key]));
					$dropship_supplier_name = sanitize_text_field($row[$dropship_supplier_name_key]);
					$dropship_supplier_description = sanitize_text_field($row[$dropship_supplier_description_key]);
					$dropship_supplier_email = sanitize_email($row[$dropship_supplier_email_key]);
					$dropship_supplier_account_number = sanitize_text_field($row[$dropship_supplier_account_number_key]);
					...
					if ($dropship_supplier) {
						...
						$term_id = is_wp_error($updated) ? $dropship_supplier->term_id : $updated['term_id'];
					} else {
					...
					if ($term_id) {
						update_term_meta($term_id, 'meta', $term_meta);
						wp_set_object_terms($pId, array($term_id), 'dropship_supplier');
					}
				} else {
					// No supplier data in CSV — remove supplier term from product
					wp_set_object_terms($pId, array(), 'dropship_supplier');
				}
			}
		}
