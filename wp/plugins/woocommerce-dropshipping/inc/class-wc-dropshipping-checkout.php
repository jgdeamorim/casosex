...
class WC_Dropshipping_Checkout {
	...
	public function init() {
		...
		add_filter( 'manage_dropshipper-order-list_columns', array( $this, 'show_pod_header' ), 1, 1 );
		...
		add_filter( 'woocommerce_new_order', array( $this, 'manual_add_cost_of_goods_on_orders' ), 10, 1 );
	...
	public function show_custom_checkout_order_page( $order ) {
		...
		if ( array_key_exists( 'checkout_order_number', $this->order_options ) ) {
			...
			if ( $this->order_options['checkout_order_number'] == 1 ) {
				...
				?> <h3><?php echo esc_html( 'Order Number' ); ?></h3>
				<?php

				echo opmc_hpos_get_post_meta( $order->get_id(), '_wc_dropshipping_order_number' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	public function show_order_number_content( $column ) {
		...
		if ( 'wc_dropshipping_order_number' === $column ) {
			...
			$order_number = opmc_hpos_get_post_meta( $order_id, '_wc_dropshipping_order_number' );
						...
						if ( is_object( $user ) ) {
							...
							if ( isset( $tracking_number ) ) {
								...
								if ( is_array( $tracking_number ) && ( '' == in_array( $supplier_nickname, $suppliers_list ) ) ) {
									...
									echo esc_html( $supplier_nickname ) . ': ' . esc_html( $tracking_number['tracking_number'] ) . '<br><br>';
					...
					foreach ( $items as $item_id => $item ) {
						...
						if ( is_object( $user ) ) {
							...
							if ( ! empty( $pod_status ) && ( '' == in_array( $supplier_nickname, $suppliers_list ) ) ) {
								...
								echo esc_html( $supplier_nickname ) . ': ' . esc_html( $pod_status ) . '<br><br>';
	...
	public function wc_view_order_number( $order_id ) {
		...
		if ( isset( $this->order_options['checkout_order_number'] ) && $this->order_options['checkout_order_number'] == 1 ) {

			$order_number = opmc_hpos_get_post_meta( $order_id->get_id(), '_wc_dropshipping_order_number' );

			if ( $order_number !== '' ) {

				echo '<span class="wcd-order-number">Order number: ' . esc_html( $order_number ) . '</span>';
	...
	public function wc_email_after_order_table( $item_id, $item, $order, $plain_text ) {
		...
		if ( isset( $this->order_options['checkout_order_number'] ) && $this->order_options['checkout_order_number'] == 1 ) {
			...
			$order_number = opmc_hpos_get_post_meta( $order->get_id(), '_wc_dropshipping_order_number' );

			if ( $order_number !== '' ) {

				echo '<p style="margin-top:20px;"><b>Order number: </b>' . esc_html( $order_number ) . '</p>';
	...
	public function product_shipping_details( $item_id, $item, $order, $plain_text ) {
		...
		if ( $order instanceof WC_Order ) {
			...
			$orders = wc_get_order( absint( $order ) );
		}

		if ( ! $orders instanceof WC_Order || ! method_exists( $orders, 'get_items' ) ) {
			return;
		}
		...
		foreach ( $orders->get_items() as $line_item_id => $line_item ) {
			...
			$supplier_id = get_post_meta( $line_item_id, 'supplierid', true );
			$arg         = array(
		...
		if ( ! empty( $arrayuser ) ) {
			...
			foreach ( $uniqe_userid as $key => $value ) {
				...
				if ( isset( $dropshipper_shipping_info ) && $dropshipper_shipping_info !== '' ) {
					echo '<h2><b>' . esc_html__( 'Track Your Order', 'woocommerce-dropshippers' ) . '</b></h2>';

					echo '<strong>' . esc_html__( 'Shipping Date', 'woocommerce-dropshippers' ) . '</strong>: <span class="dropshipper_date">' . ( empty( $dropshipper_shipping_info['date'] ) ? '-' : esc_html( $dropshipper_shipping_info['date'] ) ) . '</span><br/>' . "\n";
