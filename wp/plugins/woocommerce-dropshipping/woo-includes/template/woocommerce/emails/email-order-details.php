...
// Use dropshipping template when the plugin is active; otherwise fall back to WooCommerce core.
if (class_exists('WC_Dropshipping')) {
	...
	<div style="margin-bottom: 40px;">
		...
		$split_gst_amount = isset($options['show_gst_supplier_email']) ? $options['show_gst_supplier_email'] : '';
		?>
		<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; text-align:center;" border="1">
			...
			<tbody>
				...
				foreach ($items as $item_id => $item) {
					...
					// Ensure numeric values from the current line item only (no nested get_items loop).
					$qty          = (int) $item->get_quantity();
					$price_ex_tax = $product ? (float) wc_get_price_excluding_tax( $product ) : 0;
					$subtotal_tax = (float) $item->get_subtotal_tax();
					$total_tax    = (float) $item->get_total_tax();

					// Calculate totals
					$sub_total      = $qty * $price_ex_tax;
					$all_sub_total += $sub_total;
					$total_gst     += $subtotal_tax;

					$sku = ($product) && is_object($product) ? $product->get_sku() : ''; ?>
					 <tr>
						<td><?php echo esc_html( $item->get_name() ); ?><br /><?php echo esc_html( $sku ); ?></td>
						<td><?php echo absint( $qty ); ?></td>
						<td><?php echo wc_price( $price_ex_tax ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>

						<?php if ( 1 == $split_gst_amount ) : ?>
							<td><?php echo wc_price( $sub_total ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
							<td><?php echo wc_price( $subtotal_tax ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
							<td><?php echo wc_price( $sub_total + $total_tax ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
						<?php else : ?>
							<td style="display:none"><?php echo esc_attr( wc_price( $sub_total ) ); ?></td>
							<td style="display:none"><?php echo esc_attr( wc_price( $subtotal_tax ) ); ?></td>
							<td style="display:none"><?php echo esc_attr( wc_price( $sub_total + $total_tax ) ); ?></td>
						<?php endif; ?>
				...
				<tr style="font-weight:bold;">
					...
					<?php } ?>
					<td><?php echo wc_price(floatval($all_sub_total)); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
						?></td>
					<?php if (1 == $split_gst_amount) { ?>

						<td><?php echo wc_price(floatval($total_gst));  // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
							?></td>
						<td><?php echo wc_price(floatval((($all_sub_total + $total_gst) * 100)) / 100); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
							?></td>
