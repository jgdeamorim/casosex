...
class TCPDF {
	...
	public function Error($msg) {
		...
		if (defined('K_TCPDF_THROW_EXCEPTION_ERROR') AND !K_TCPDF_THROW_EXCEPTION_ERROR) {
			die('<strong>TCPDF ERROR: </strong>'.$msg); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		} else {
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					echo $this->getBuffer(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				break;
			}
			case 'D': {
	...
	public function setProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if ($mode > 1) { // AES
			...
			if (!extension_loaded('openssl') || !in_array('aes-128-cbc', openssl_get_cipher_methods())) {
				$this->Error('AES encryption requires the OpenSSL extension with AES-128-CBC cipher.');
			}
	...
	protected function getHtmlDomArray($html) {
		...
		$csstagarray = '<cssarray>' . htmlentities(json_encode($css), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401, 'UTF-8') . '</cssarray>';
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					if (($imgsrc[0] === '/') AND !empty($_SERVER['DOCUMENT_ROOT']) AND ($_SERVER['DOCUMENT_ROOT'] != '/')) {
						// fix image path
						$findroot = strpos($imgsrc, $_SERVER['DOCUMENT_ROOT']); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
						if (($findroot === false) OR ($findroot > 1)) {
							if (substr($_SERVER['DOCUMENT_ROOT'], -1) == '/') { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
								$imgsrc = substr($_SERVER['DOCUMENT_ROOT'], 0, -1).$imgsrc; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
							} else {
								$imgsrc = $_SERVER['DOCUMENT_ROOT'].$imgsrc; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
							}
						}
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						if (($img[0] == '/') AND !empty($_SERVER['DOCUMENT_ROOT']) AND ($_SERVER['DOCUMENT_ROOT'] != '/')) {
							$findroot = strpos($img, $_SERVER['DOCUMENT_ROOT']); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
							if (($findroot === false) OR ($findroot > 1)) {
								if (substr($_SERVER['DOCUMENT_ROOT'], -1) == '/') { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
									$img = substr($_SERVER['DOCUMENT_ROOT'], 0, -1).$img; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
								} else {
									$img = $_SERVER['DOCUMENT_ROOT'].$img; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
								}
							}
						}
						$img = urldecode($img);
