	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function _AES($key, $text) {
		...
		if (extension_loaded('openssl')) {
			...
			$encrypted = openssl_encrypt($text, $algo, $key, OPENSSL_RAW_DATA, $iv);			
			return $iv . substr($encrypted, 0, -16);
		}
	...
	public static function _RC4($key, $text, &$last_enc_key, &$last_enc_key_c) {
		...
		if (function_exists('openssl_encrypt')) {
			// Use OpenSSL RC4 equivalent
			$out = @openssl_encrypt($text, 'rc4', $key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING);
			if ($out !== false) {
				return $out;
			}
		}
	...
	public static function fileGetContents($file) {
		...
		if ((strlen($file) > 1)
		    ...
		    $findroot = strpos($file, $_SERVER['DOCUMENT_ROOT']); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		    if (($findroot === false) || ($findroot > 1)) {			
				$alt[] = htmlspecialchars_decode(urldecode($_SERVER['DOCUMENT_ROOT'] . $file), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		    }
		}
		
		$protocol = 'http';
		if (!empty($_SERVER['HTTPS']) && (strtolower($_SERVER['HTTPS']) != 'off')) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		    $protocol .= 's';
		}
		...
		$url = htmlspecialchars_decode($url, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401);
		$alt[] = $url;
		//
		if (preg_match('%^(https?)://%', $url)
			...
			if (empty($urldata['query'])) {
				$host = $protocol.'://'.$_SERVER['HTTP_HOST']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				if (strpos($url, $host) === 0) {
				    // convert URL to full server path
				    $tmp = str_replace($host, $_SERVER['DOCUMENT_ROOT'], $url); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				    $alt[] = htmlspecialchars_decode(urldecode($tmp), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401);
				}
			}
		}
		...
		if (isset($_SERVER['SCRIPT_URI'])
		    ...
		    $urldata = @parse_url($_SERVER['SCRIPT_URI']); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
