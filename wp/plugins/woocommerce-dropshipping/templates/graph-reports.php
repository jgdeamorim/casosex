...
/** @var WC_Dropshipping $this */
   ...
   $wcd_graph_points = static function ( $rows ) {
	   $points = array();

	   foreach ( (array) $rows as $row ) {
		   $points[] = array(
			   'x' => isset( $row->post_date ) ? $row->post_date : '',
			   'y' => isset( $row->total ) ? (float) $row->total : 0,
		   );
	   }
   ...
   $wcd_graph_data = array(
	   'day'    => $wcd_graph_points( $get_one_week_daily_total_orders ),
	   'day1'   => $wcd_graph_points( $get_one_week_daily_profit ),
	   'week'   => $wcd_graph_points( $get_per_week_total_orders ),
	   'week1'  => $wcd_graph_points( $get_per_week_profit ),
	   'month'  => $wcd_graph_points( $get_one_month_total_orders ),
	   'month1' => $wcd_graph_points( $get_per_month_profit ),
	   'year'   => $wcd_graph_points( $get_per_year_total_orders ),
	   'year1'  => $wcd_graph_points( $get_per_year_profit ),
   );
...
<section class="p_wrapper">
	...
	// setup 
	  ...
	  const day = <?php echo wp_json_encode( $wcd_graph_data['day'] ); ?>;
	  const day1 = <?php echo wp_json_encode( $wcd_graph_data['day1'] ); ?>;
	  const week = <?php echo wp_json_encode( $wcd_graph_data['week'] ); ?>;
	  const week1 = <?php echo wp_json_encode( $wcd_graph_data['week1'] ); ?>;
	  const month = <?php echo wp_json_encode( $wcd_graph_data['month'] ); ?>;
	  const month1 = <?php echo wp_json_encode( $wcd_graph_data['month1'] ); ?>;
	  const year = <?php echo wp_json_encode( $wcd_graph_data['year'] ); ?>;
	  const year1 = <?php echo wp_json_encode( $wcd_graph_data['year1'] ); ?>;
