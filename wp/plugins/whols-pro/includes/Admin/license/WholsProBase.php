...
if(!class_exists("Whols_Pro_Base")) {
	class Whols_Pro_Base {
    	public $key = "2C4B1CAFD9F05354";
    	private $product_id = "19";
    	private $product_base = "whols-pro";
		...
		function __construct($plugin_base_file='')
		{
			...
			$this->plugin_file= PL_FILE; // Updated this by plugin file to show the "Update Check" button
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_whols-pro_fupc', function(){
					...
					if($this->is_theme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=whols-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		public function isWooLentorBundle(){
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..","whols-pro").'</h1>');
		...
		function el_plugin_update_info(){
            if(function_exists("wp_remote_get")) {
                ...
                // Early return for cached failures
                ...
                if ( ! $old_found ) {
                    ...
                    // Cache failure to avoid retrying on every update check
                    if (is_wp_error($response)) {
                        set_transient( $this->product_base."_up", [ "error" => true ], 6 * HOUR_IN_SECONDS );
                        return null;
                    }
                }
		...
		final function _check_wp_plugin( $purchase_key, &$error = "", &$response_obj = null ) {
            ...
            if(!empty($old_respons)) {
                ...
                // If previous request failed but cooldown hasn't expired, don't retry
                if ( ! $is_force && empty( $old_respons->is_valid ) && ! empty( $old_respons->next_request ) && $old_respons->next_request > time() ) {
                    $error = "License server temporarily unavailable";
                    return false;
                }
            }
                        ...
                        if ( ! empty( $response->data ) ) {
                            ...
                            if ( $license_obj->is_valid ) {
                                ...
                                if ( $this->_check_old_tied( $old_respons, $response_obj, $response ) ) {
                                    ...
                                    $this->_save_failed_response( $purchase_key );
        ...
        private function _save_failed_response( $purchase_key ) {
            $failed_response = new stdClass();
            $failed_response->is_valid = false;
            $failed_response->next_request = strtotime("+ 6 hour");
            $failed_response->license_key = $purchase_key;
            $failed_response->tried = 3;
            $this->save_wp_response( $failed_response );
        }
